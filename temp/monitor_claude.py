...
def get_bgp_current_status(db: DatabaseManager) -> list:
    if db is None:
        return []
...
def get_recently_changed_peers(db: DatabaseManager, time_window_hours: int = 12):
    ...
    bgp = db.execute_query(
        "SELECT * FROM bgp_state_changes WHERE last_updated_ts >= ?", (since,)
    )
    ospf = db.execute_query(
        "SELECT * FROM ospf_state_changes WHERE last_updated_ts >= ?", (since,)
    )
    return bgp, ospf
...
def get_persistent_non_full_peers(db: DatabaseManager) -> list:
    ...
    """)
    full_keys = {(r[0], r[1], r[2]) for r in cursor.fetchall()}
