...
def setup_database(db_path):
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS bgp_peer_status
        (hostname TEXT, host_ip TEXT, vpn_instance TEXT, local_router_id TEXT, local_as_number TEXT, neighbor_ip TEXT, peer_as TEXT, up_down_time TEXT, state TEXT, last_updated_ts TEXT, 
...
def process_log_file(conn, log_file_path, file_id, log_dir_base):
    ...
    cursor.execute("DELETE FROM bgp_peer_status WHERE hostname = ?", (hostname,))
    cursor.execute("DELETE FROM ospf_peer_status WHERE hostname = ?", (hostname,))
    ...
    if vendor == 'hpe':
        ...
        # hpe_ospf_log_regex = re.compile(r"%(\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}:\d{3}).*?OSPF/5/OSPF_NBR_CHG.*?OSPF\s+(\d+).*?Neighbor\s+([\d\.]+)\(([\w\.-]+)\)\s+changed from\s+([\w/]+)\s+to\s+([\w/]+)")        
        # hpe_ospf_log_regex = re.compile(r"%(\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}:\d{3}).*?OSPF_NBR_CHG:.*?OSPF\s+(\d+).*?Neighbor\s+([\d\.]+)\(([\w\.-]+)\)\s+changed from\s+([\w/]+)\s+to\s+([\w/]+)")
    ...
    if isinstance(routing_info.get("BGP"), list):
        data_rows = "hostname, host_ip, vpn_instance, local_router_id, local_as_number, neighbor_ip, peer_as, up_down_time, state, last_updated_ts, last_snapshot_id, source_log_file"
        ...
        for bgp_instance in routing_info["BGP"]:
            ...
            for peer in bgp_instance.get("Peer", []): # 12 rouws
                values_to_insert = (hostname, host_ip, vpn_instance, local_router_id, local_as_number, peer.get("neighbor_ip"), peer.get("peer_AS"), peer.get("peer_uptime"), peer.get("peer_status"), last_updated_ts, last_snapshot_id, relative_log_path)
                cursor.execute(f'''INSERT OR REPLACE INTO bgp_peer_status 
                                ( {data_rows} ) VALUES ( {placeholders} )''', values_to_insert)
    ...
    if isinstance(routing_info.get("OSPF"), list):
        ...
        for ospf_process in routing_info["OSPF"]:
            ...
            for neighbor in ospf_process.get("neighbors", []):
                ...
                cursor.execute(f'''INSERT OR REPLACE INTO ospf_peer_status 
                                ({data_rows}) VALUES ({placeholders})''', values_to_insert)
                # logger.debug(f"Inserted OSPF peer: {neighbor.get('neighbor_routerid')} on interface {neighbor.get('Interface')} with last_down_time: {event_data.get('last_time') if event_data else 'None'}")
                ...
                if vendor == "cisco":                
                    ...
                    if neighbor_match:
                        ...
                        if current_neighbor.get("Area"):
                            current_ospf_process = {"process": None, "process_routerid": None, "neighbors": [], "lastevents": {}}
                            routing_info["OSPF"].append(current_ospf_process)                                   
                            current_ospf_process["neighbors"].append(current_neighbor)
                            current_neighbor = None
                    elif line.strip() == "":
...
def main(log_file_path=None):
    ...
    log_directory = mainconfig.LOGS_DIR / 'core_logs'
    database_path = mainconfig.DATA_DIR / 'network_analysis.db'
