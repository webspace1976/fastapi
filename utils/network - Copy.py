        ...
        try:
            ...
            <table id="{ip}" style="width:100%;">
                <tr style="display:flex;">
                    <td style="width:75%;"><b>{hostname}</b></td>
                    <td style=\"width:25%\">{label}: <a href=\"\\logs\\core_logs\\{fname}\" target=\"_blank\">{fname}</a>
                </tr>
            """) 
    ...
    if os.path.basename(os.path.dirname(log_file_path)) != "arch" or not os.path.exists(output_json_path):     # for normal log file, save for report every time; or the first time for archived log file
        with open(log_file_path, 'r') as file:
            ...
            for line in file:
                ...
                #     if line.startswith("%"):  # New log entry
                #         if current_entry:
                #             log_entries.append(current_entry)
                #         current_entry = line.rstrip()
                #     elif line.startswith(" "):  # Indented line, continuation
                #         if current_entry is not None:
                #             current_entry += "\n" + line.rstrip()
                #     else:
                #         # Not a log line or continuation, ignore or handle as needed
                #         pass
                if current_section == 'log':
                    if current_os == 'hpe':
                        if line.startswith("%"):
